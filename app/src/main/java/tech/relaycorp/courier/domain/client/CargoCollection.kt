...
class CargoCollection
    ...
    constructor(
        ...
        private suspend fun storeCargo(data: InputStream) = storeMessage.storeCargo(data, GatewayType.Private)

        private suspend fun deleteCCA(cca: StoredMessage) = deleteMessage.delete(cca)

        @Throws(MessageDataNotFoundException::class)
        private suspend fun StoredMessage.getSerializedInputStream() = diskRepository.readMessage(storagePath)
    }
