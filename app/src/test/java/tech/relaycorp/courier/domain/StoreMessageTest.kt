...
class StoreMessageTest {
    ...
    @Test
    internal fun `store message with just enough space`() = runTest {
    ...
    inner class StoreCargo {
        @Test
        fun `Malformed cargo should not be stored`() = runTest {
            val result =
        ...
        @Test
        fun `Well-formed yet invalid cargo should not be stored`() = runTest {
        ...
        inner class BoundForInternetGateway {
            ...
            @Test
            fun `Cargo should be refused if Internet gateway of recipient is missing`() = runTest {
                val invalidCargo = Cargo(
            ...
            @Test
            fun `Valid cargo should be stored`() = runTest {
                val cargo = Cargo(
                    recipient,
                    "payload".toByteArray(),
                    CDACertPath.PRIVATE_GW,
            ...
            @Test
            fun `Delivery authorization should not be required`() = runTest {
                val cargo = Cargo(
        ...
        inner class BoundForPrivateGateway {
            ...
            fun `Unauthorized cargo should be refused`() = runTest {
                val cargo = Cargo(
                    recipient.copy(id = "${recipient.id}abc"),
                    "payload".toByteArray(),
            ...
            @Test
            fun `Authorized cargo should be accepted`() = runTest {
                val cargo = Cargo(
                    recipient,
                    "payload".toByteArray(),
                    CDACertPath.INTERNET_GW,
    ...
    inner class StoreCCA {
        @Test
        fun `do not store malformed CCA`() = runTest {
        ...
        @Test
        fun `do not store well-formed yet invalid CCA`() = runTest {
